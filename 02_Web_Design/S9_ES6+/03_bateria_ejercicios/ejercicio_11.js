...
const favoriteSoundsCount = {};

for (const user of users) {
  for (const sound in user.favoritesSounds) {
    if (favoriteSoundsCount[sound]) {
      favoriteSoundsCount[sound]++;
    } else {
      favoriteSoundsCount[sound] = 1;
    }
  }
}

console.log("Conteo de veces que cada sonido ha sido agregado como favorito:");
for (const sound in favoriteSoundsCount) {
  console.log(`${sound}: ${favoriteSoundsCount[sound]} vez(es)`);
}
